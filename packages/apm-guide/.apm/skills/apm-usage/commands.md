...
| `apm self-update` | Update the APM CLI itself (or show distributor guidance when self-update is disabled at build time). Prior to v0.12.5 this lived under `apm update`; the old verb still works for one release outside an apm.yml project, then will be removed. | `--check` only check |
