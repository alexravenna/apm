"""Legacy link resolution helpers (backward-compatibility layer).

These module-level functions predate :class:`UnifiedLinkResolver` and are
kept purely for backward-compatibility. New code should use
:class:`UnifiedLinkResolver` directly.

.. note::
    Symbols are re-exported from ``apm_cli.compilation.link_resolver`` so
    all existing ``from apm_cli.compilation.link_resolver import …`` imports
