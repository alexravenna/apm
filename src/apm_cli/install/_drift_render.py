"""Human- and machine-readable renderers for drift findings.

Formats supported:

* ``text``  -- plain text grouped by kind (default, human-readable).
* ``json``  -- ``{"drift": [...]}`` machine-readable JSON object.
* ``sarif`` -- SARIF ``results`` array; rule IDs use ``apm/drift/<kind>``.
