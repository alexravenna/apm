...
def _apply_transport_overlay(info: dict, dep) -> None:
    """Select matching transport from available options (in-place)."""
    if not dep.transport:
        return
    if dep.transport in ("http", "sse", "streamable-http"):
        if info.get("remotes"):
...
def _apply_headers_overlay(info: dict, dep) -> None:
    """Merge dep.headers into remote headers (in-place)."""
    if not dep.headers or "remotes" not in info:
        return
...
def _apply_args_overlay(info: dict, dep) -> None:
    """Merge dep.args into package runtime_arguments (in-place)."""
    if not dep.args or "packages" not in info:
        return
...
def _warn_unimplemented_overlay_fields(dep) -> None:
...
def _apply_overlay(server_info_cache: dict, dep) -> None:
    ...
    _apply_transport_overlay(info, dep)
    ...
    _apply_headers_overlay(info, dep)
    _apply_args_overlay(info, dep)

    # Tools overlay: embed for adapters to pick up
    if dep.tools:
        info["_apm_tools_override"] = dep.tools

    _warn_unimplemented_overlay_fields(dep)
