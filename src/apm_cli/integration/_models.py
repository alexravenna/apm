"""Data-model classes for target profiles.

:class:`PrimitiveMapping` and :class:`TargetProfile` are kept here so that
``targets.py`` can stay under 500 lines while still exposing them under the
canonical ``apm_cli.integration.targets`` namespace (via a re-import there).

Do **not** import ``KNOWN_TARGETS`` or any integration-layer module from
this file -- the models must remain import-cycle–free.
"""
