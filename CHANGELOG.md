...
- **Top-level `apm add` / `apm remove` for marketplace sources.** Promoted the legacy `apm marketplace add`/`apm marketplace remove` surface to first-class commands so plugin source registration matches the muscle memory of `npm install` / `brew tap` / Claude Code's `/plugin marketplace add`. New behaviour: multi-source registration (`apm add github/awesome-copilot microsoft/azure-skills`), smart typo error for bare names without a slash (suggests `apm install`), and fail-closed batch on security-class errors (path traversal). The legacy commands keep working and emit a one-line stderr deprecation tip on success pointing to the new alias. (#1075)
